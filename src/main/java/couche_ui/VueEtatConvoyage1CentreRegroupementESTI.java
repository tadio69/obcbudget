 ...
 */
public class VueEtatConvoyage1CentreRegroupementESTI extends javax.swing.JFrame {

    /**
     * Creates new form VueEtatConvoyage1
     */
    public VueEtatConvoyage1CentreRegroupementESTI() {
        initComponents();
        doMyInit();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("DES CENTRES DE REGROUPEMENT ESTI");
        ...
        if (!groupedexamensEnstring.equals("") && !anneeString.equals("---")) {
            ...
            if (!continuer) {
                ...
                try {
                            ...
                            if (region.getNomreg().equals(centrebilletage.getEtablissement().getArrondissement().getDepartement().getRegion().getNomreg())) {
                                ...
                                for (int i = 0; i < souscentresList.size(); i++) {
                                    ...
                                    if (souscentreEffectifMap.containsKey(souscentre)) {
                                        //le souscentre doit être centre de regroupement et non ventilation
                                        if (testSiCentreDeRegroupement(souscentre) && !testSiCentreDeVentilation(souscentre)) {
                                            //le souscentre doit être centre de regroupement                                                                                                                                                    //on recherche ledit centreregroupement
                                            //on le cherche et on calcule le coût
                                            ...
                                            determinerCoutESTI(centreregroupement, centreregroupementEffectifMap);
                                            ...
                                            String souscentreAffich = souscentre.getNomsouscent();
                                            if (!souscentreAsecretariatsDemultipliesList.contains(souscentre)) {
                                                //on prend les activités de son secrétariat 1
                                                phasesList = activitesSecretariatMap.get(souscentreSecretariatsMap.get(souscentre).get(0));
                                                if (!phasesList.isEmpty()) {
                                                    for (char c : phasesList) {
                                                        if (c != 'T') {
                                                            phasesEnchaine += c;
                                                ...
                                                souscentreAffich =souscentre.getNomsouscent()+" sec. 1";
                                            }
                                            ...
                                            row.put("num", num++);
                                            row.put("souscentre", souscentreAffich + "\n\n" + nbcolisEnchaine);
                    ...
                    JasperReport jasperReport = JasperCompileManager.compileReport("src\\main\\java\\reports\\etatConvoyage1ESTIregroupementReport.jrxml");
            ...
            if (!caisses.isEmpty()) {
                ...
                try {
                    for (Souscentre souscentre : souscentreEffectifMap.keySet()) {
                        //le souscentre doit être centre de regroupement et non ventilation
                        if (testSiCentreDeRegroupement(souscentre) && !testSiCentreDeVentilation(souscentre)) {
                            //nbtours = 1 car si le soucentre est démultiplié on prend son sec 1                           
                            //on recherche ledit centreregroupement  
                            //que le souscentre soit démultiplié ou pas c'est le même cacul sauf qu'à l'affichage on mettra sec 1 en lieu et place du souscentre
                            Centredispatching centreregroupement = metier.findCentredispatchingD1TypeByEtablissement(souscentre.getEtablissement().getAbreviation(), "Regroupement");
                            determinerCoutESTI(centreregroupement, centreregroupementEffectifMap);
                            int totalCMArrondi = (int) Math.ceil((double) totalCM / (double) 1000) * 1000;
                            int totalChefsecArrondi = (int) Math.ceil((double) totalChefsec / (double) 1000) * 1000;
    ...
    private void determinerCoutESTI(Centredispatching centredispatching, Map<Centredispatching, Integer> centredispatchingEffectifMap) {
        distance = 0;
        indemchefsec = 0;
        //ici on considère uniquement les centres de regroupement
    ...
    private boolean testSiCentreDeRegroupement(Souscentre souscentre) {
        //NB: un centre de regroupement ne bénéficie pas des frais de convoyage 1 à ESG
